            ...
            <div class="col-md-10">
                <div class="card">
                    <div class="card-header">
                        <a class="h3"> {{ __("Add New Contest") }} </a>
                    </div>

                    <div class="card-body">


                        <form method="post"  action="{{ route('admin.contest.store') }}" onsubmit="return vue.handleSubmitCheck()">
                            @csrf
...
@section('script')
    <script>
        let data = {
            data: function () {
                return {
                    contestTime: '{{ old('contest-time') }}',
                    studentsRank: JSON.parse('{!! old('contest-rank') === null ? '[]' : old('contest-rank') !!}'),
            ...
            methods: {
                ...
                deleteRow(index) {
                    ...
                    // this.$confirm('Are you sure Delete?', 'Notice', {
                    //     confirmButtonText: 'Confirm',
                    //     cancelButtonText: 'Cancel',
                    //     type: 'warning'
                    // }).then(() => {this.studentsRank.splice(index, 1)})
                    //     .catch(() => {this.$message({type: 'info', message: 'Cancel'})})
                },
